        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {VISION_ITEMS.map((item) => (
            <a
              key={item.id}
              href={item.link}
              className="group relative block h-96 rounded-2xl overflow-hidden shadow-lg hover:shadow-2xl hover:shadow-black/30 transition-all duration-500 ease-out transform hover:-translate-y-4 hover:scale-105 cursor-pointer"
            >
              ...
              <div className="absolute inset-x-0 bottom-0 p-6 z-20">
                ...
                <div className="bg-black/60 backdrop-blur-sm rounded-xl p-4 transform group-hover:bg-black/70 transition-all duration-300">
                  {/* Title - BIGGER and BOLDER */}
                  <h3 className="text-2xl md:text-3xl font-serif font-black text-white leading-tight drop-shadow-lg">
                    {item.title}
                  </h3>
